$(function ($) {
    ...
    obj.ajax_get_modal_content = function (
        ajax_url,
        $modal,
        computer_name,
        method
    ...
    function ajax_get_computer_details_for_history(
        computer_name,
        method,
        $modal
    ) {
        ...
        obj.computer_history = $("#computer-history").DataTable({
            ...
            ajax:
                "ajax_computer/get_computer_history_details_for_table/" +
                computer_name,
            deferRender: true,
            order: [[0, "desc"]],
            columns: [
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        console.log("row", row);
                        return getUnitStatusDescription(row.unit_status);
                    },
                },
            ],
            language: {
    ...
    function load_modal(url, computer_name, method) {
        ...
        obj.ajax_get_modal_content(
            ajax_url,
            $modal,
            computer_name,
            method
        ).done(function () {
            ajax_get_computer_details_for_history(
                computer_name,
                method,
                $modal
            );
